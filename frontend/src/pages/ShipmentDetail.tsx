...
function SeverityBadge({ status }: { status: "High" | "Attention" | "Safe" }) {
  const style = status === "High" ? "border-[#6e3c3b] bg-[#281819] text-[#e9918d]" : status === "Attention" ? "border-[#655129] bg-[#272115] text-[#e4c177]" : "border-[#315a48] bg-[#16271f] text-[#78c8a5]";
  return <span className={`inline-flex rounded border px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.1em] ${style}`}>{status}</span>;
}
...
export default function ShipmentDetail() {
  ...
  return (
    <main className="min-h-screen bg-[#0b1117] font-sans text-slate-200">
      <header className="sticky top-0 z-30 flex h-[64px] items-center justify-between border-b border-slate-800 bg-[#0d151d] px-4 sm:px-6">
        <div className="flex items-center gap-3"><Link href="/dashboard/admin?view=shipments" className="grid h-9 w-9 place-items-center rounded border border-slate-700 text-slate-300 transition hover:bg-slate-800 hover:text-white" aria-label="Back to Live Shipments"><ArrowLeft className="h-4 w-4" /></Link><div className="flex items-center gap-2.5"><img src="/manus-storage/cold-chain-mark_8a9c38e3.png" alt="Cold Chain AI" className="h-8 w-8" /><div><p className="font-display text-[13px] font-bold uppercase tracking-[0.12em] text-white">Cold Chain <span className="text-[#73c8a7]">AI</span></p><p className="text-[8px] font-bold uppercase tracking-[0.2em] text-slate-500">Ops / Control</p></div></div></div>
        <div className="flex items-center gap-2"><button type="button" className="relative grid h-9 w-9 place-items-center rounded border border-slate-700 text-slate-300"><Bell className="h-4 w-4" /><span className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-[#d56560]" /></button><span className="grid h-8 w-8 place-items-center rounded bg-[#238767] text-[10px] font-bold text-white">AM</span></div>
      </header>
