...
export default function AdminDashboard() {
  ...
  const viewMeta: Record<Workspace, { eyebrow: string; title: string; description: string }> = {
    ...
    "Live Shipments": { eyebrow: "LIVE SHIPMENTS", title: "Active shipments", description: "Search, filter, and open the shipment that needs attention." },
    "Risk Monitor": { eyebrow: "RISK MONITOR", title: "Shipment health", description: "Review temperature exposure and remaining safe life by priority." },
    "Recommended Actions": { eyebrow: "ACTION QUEUE", title: "Recommended actions", description: "Review operations that require human confirmation." },
    "Route Planning": { eyebrow: "ROUTE PLANNING", title: "Intervention comparison", description: "Compare options before dispatching a change." },
    "Product Profiles": { eyebrow: "PRODUCT PROFILES", title: "Temperature profiles", description: "Reference operating ranges for monitored products." },
    "Audit Reports": { eyebrow: "AUDIT REPORTS", title: "Shipment records", description: "Review the event history available for export." },
  };
    ...
    <main className="min-h-screen bg-[#0b1117] font-sans text-slate-200">
      ...
      <div className="flex">
        <aside className={`${mobileNav ? "fixed inset-x-0 top-[64px] z-30 block border-b border-slate-800" : "hidden"} w-full shrink-0 bg-[#0d151d] p-3 lg:sticky lg:top-[64px] lg:block lg:h-[calc(100vh-64px)] lg:w-[230px] lg:border-b-0 lg:border-r lg:border-slate-800`}>
          ...
          <nav className="space-y-1">{navItems.map(([label, NavIcon]) => { const Icon = NavIcon; const selected = activeView === label; return <button type="button" key={label} onClick={() => changeView(label)} className={`flex min-h-10 w-full items-center gap-3 rounded px-3 text-left text-sm font-medium transition ${selected ? "bg-[#18362d] text-[#7bd0af]" : "text-slate-400 hover:bg-slate-800/70 hover:text-slate-100"}`}><Icon className="h-4 w-4" /> {label}</button>; })}</nav>
          <div className="mt-6 border-t border-slate-800 pt-4"><button type="button" className="flex min-h-10 w-full items-center gap-3 px-3 text-sm font-medium text-slate-500"><Settings2 className="h-4 w-4" /> Settings</button></div>
        </aside>
        ...
        <section className="min-w-0 flex-1 p-4 sm:p-6 lg:p-8">
          <div className="mx-auto max-w-[1480px]">
            ...
            {activeView === "Live Shipments" && <><Panel className="mt-5 p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><label className="relative block max-w-xl flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" /><input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} className="h-10 w-full rounded border border-slate-700 bg-[#0d161e] pl-9 pr-3 text-sm text-white outline-none placeholder:text-slate-600 focus:border-[#278a69]" placeholder="Search by shipment, product, or destination" /></label><div className="flex flex-wrap gap-2">{(["All", "High", "Attention", "Safe"] as const).map((filter) => <button type="button" key={filter} onClick={() => setShipmentFilter(filter)} className={`min-h-9 rounded border px-3 text-xs font-semibold ${shipmentFilter === filter ? "border-[#2d7660] bg-[#17362c] text-[#82d1b0]" : "border-slate-700 text-slate-400 hover:bg-slate-800"}`}>{filter === "All" ? "All shipments" : filter}</button>)}</div></div></Panel><div className="mt-5 grid gap-5 xl:grid-cols-[1fr_0.68fr]"><Panel className="overflow-hidden"><div className="border-b border-slate-800 px-5 py-4"><p className="text-sm font-semibold text-white">{filteredShipments.length} shipments</p><p className="mt-1 text-xs text-slate-500">Select a row to open the shipment record.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="border-b border-slate-800 bg-[#0d161e] text-[10px] font-bold uppercase tracking-[0.08em] text-slate-500"><tr><th className="px-5 py-3">Shipment</th><th className="px-5 py-3">Route</th><th className="px-5 py-3">Condition</th><th className="px-5 py-3">ETA</th><th className="px-5 py-3">Status</th></tr></thead><tbody>{filteredShipments.map((shipment) => <tr key={shipment.id} onClick={() => openShipment(shipment)} className="cursor-pointer border-b border-slate-800/80 text-slate-300 transition hover:bg-slate-800/50"><td className="px-5 py-4"><p className="font-semibold text-white">{shipment.id}</p><p className="mt-1 text-xs text-slate-500">{shipment.product} · {shipment.batch}</p></td><td className="px-5 py-4 text-xs"><p>{shipment.origin}</p><p className="mt-1 text-slate-500">to {shipment.destination}</p></td><td className="px-5 py-4"><p className="font-medium text-white">{shipment.temperature}</p><p className="mt-1 text-xs text-slate-500">Safe life: {shipment.safeLife}</p></td><td className="px-5 py-4 font-medium">{shipment.eta}</td><td className="px-5 py-4"><RiskBadge status={shipment.status} /></td></tr>)}</tbody></table></div></Panel><Panel className="p-5"><p className="text-xs font-semibold text-slate-300">Dispatch summary</p><div className="mt-5 space-y-4"><div className="border-b border-slate-800 pb-4"><p className="text-3xl font-semibold text-white">24</p><p className="mt-1 text-sm text-slate-500">active shipments in the network</p></div><div className="grid grid-cols-3 gap-3 text-center"><div><p className="text-xl font-semibold text-[#e9918d]">3</p><p className="mt-1 text-[10px] uppercase tracking-[0.08em] text-slate-500">High</p></div><div><p className="text-xl font-semibold text-[#e4c177]">7</p><p className="mt-1 text-[10px] uppercase tracking-[0.08em] text-slate-500">Attention</p></div><div><p className="text-xl font-semibold text-[#72c8a5]">14</p><p className="mt-1 text-[10px] uppercase tracking-[0.08em] text-slate-500">Safe</p></div></div></div></Panel></div></>}

            {activeView === "Live Shipments" && liveDetailOpen && <Panel className="mt-5 p-5"><div className="flex flex-col gap-4 border-b border-slate-800 pb-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#74c8a7]">Shipment health</p><h2 className="font-display mt-2 text-3xl font-bold tracking-[-0.06em] text-white">{selectedShipment.id}</h2><p className="mt-1 text-sm text-slate-400">{selectedShipment.product} · Batch {selectedShipment.batch} · {selectedShipment.origin} → {selectedShipment.destination}</p></div><div className="flex items-center gap-3"><RiskBadge status={selectedShipment.status} /><button type="button" onClick={() => setLiveDetailOpen(false)} className="grid h-9 w-9 place-items-center rounded border border-slate-700 text-slate-400 hover:bg-slate-800 hover:text-white" aria-label="Close shipment health panel"><X className="h-4 w-4" /></button></div></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="border border-slate-800 bg-[#0d161e] p-4"><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500">Health score</p><p className="font-display mt-2 text-4xl font-bold tracking-[-0.07em] text-white">{selectedShipment.healthScore}<span className="text-lg text-slate-500">/100</span></p><div className="mt-3 h-1.5 overflow-hidden bg-slate-800"><span className="block h-full bg-[#278a69]" style={{ width: `${selectedShipment.healthScore}%` }} /></div></div><div className="border border-slate-800 bg-[#0d161e] p-4"><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500">Spoilage risk</p><p className="font-display mt-2 text-4xl font-bold tracking-[-0.07em] text-[#e9918d]">{selectedShipment.risk}%</p><p className="mt-3 text-xs text-slate-500">Based on current exposure.</p></div><div className="border border-slate-800 bg-[#0d161e] p-4"><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500">Safe life remaining</p><p className="font-display mt-2 text-4xl font-bold tracking-[-0.07em] text-white">{selectedShipment.safeLife}</p><p className="mt-3 text-xs text-slate-500">At current conditions.</p></div></div><div className="mt-5 grid gap-5 lg:grid-cols-[1fr_1fr]"><div className="border-l-2 border-[#278a69] pl-4"><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#74c8a7]">Current condition</p><div className="mt-3 flex flex-wrap items-end gap-x-6 gap-y-3"><div><p className="text-xs text-slate-500">Temperature</p><p className="font-display mt-1 text-3xl font-bold tracking-[-0.06em] text-white">{selectedShipment.temperature}</p></div><div><p className="text-xs text-slate-500">Operating range</p><p className="mt-1 text-lg font-semibold text-[#74c8a7]">2–6°C</p></div><div><p className="text-xs text-slate-500">ETA</p><p className="mt-1 text-lg font-semibold text-white">{selectedShipment.eta}</p></div></div></div><div className="border-l-2 border-[#d6a855] pl-4"><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#e4c177]">Operator note</p><p className="mt-3 text-sm leading-6 text-slate-400">{selectedShipment.status === "Safe" ? "Shipment is within the configured range. Continue routine monitoring." : "Temperature exposure is consuming safe life. Review the recommended intervention before the next scan."}</p><button type="button" onClick={() => { setLiveDetailOpen(false); setActiveView("Dashboard"); }} className="mt-4 min-h-9 rounded border border-[#2e7660] bg-[#133329] px-3 text-xs font-semibold text-[#8ad8bb] hover:bg-[#1a4034]">Open full record</button></div></div></Panel>}

            {activeView === "Risk Monitor" && <><div className="mt-5 grid gap-3 sm:grid-cols-3">{[["High risk", "3", "Review now", "#e9918d"], ["Attention", "7", "Check within 30 min", "#e4c177"], ["Within range", "14", "No action needed", "#72c8a5"]].map(([label, count, note, color]) => <Panel key={label as string} className="p-4"><p className="text-xs font-medium text-slate-400">{label as string}</p><p className="mt-3 text-3xl font-semibold" style={{ color: color as string }}>{count as string}</p><p className="mt-1 text-xs text-slate-500">{note as string}</p></Panel>)}</div><div className="mt-5 grid gap-5 xl:grid-cols-[1fr_0.68fr]"><Panel className="p-5"><div className="flex items-center justify-between"><div><p className="text-sm font-semibold text-white">Priority risks</p><p className="mt-1 text-xs text-slate-500">Ordered by exposure and time remaining.</p></div><button type="button" onClick={() => setShipmentFilter("All")} className="flex min-h-9 items-center gap-2 rounded border border-slate-700 px-3 text-xs font-semibold text-slate-300 hover:bg-slate-800"><Filter className="h-3.5 w-3.5" /> Reset</button></div><div className="mt-4 divide-y divide-slate-800">{shipments.filter((shipment) => shipment.status !== "Safe").map((shipment) => <button type="button" key={shipment.id} onClick={() => openShipment(shipment)} className="grid w-full grid-cols-[1fr_auto] gap-4 py-4 text-left transition hover:bg-slate-800/35"><div><div className="flex items-center gap-2"><p className="font-semibold text-white">{shipment.id}</p><RiskBadge status={shipment.status} /></div><p className="mt-2 text-sm text-slate-300">{shipment.product} · {shipment.temperature} · {shipment.origin} → {shipment.destination}</p><p className="mt-1 text-xs text-slate-500">Safe life remaining: {shipment.safeLife}</p></div><div className="text-right"><p className="text-2xl font-semibold text-[#e9918d]">{shipment.risk}%</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[0.08em] text-slate-500">Risk</p></div></button>)}</div></Panel><Panel className="p-5"><p className="text-sm font-semibold text-white">Risk checks</p><div className="mt-5 space-y-4">{[["Temperature exposure", "3 shipment(s) outside product range", "#e9918d"], ["Rising temperature", "2 shipment(s) with upward trend", "#e4c177"], ["Short safe life", "1 shipment(s) under 45 minutes", "#e9918d"], ["Device coverage", "All live shipments reporting", "#72c8a5"]].map(([title, note, color]) => <div key={title as string} className="flex gap-3"><span className="mt-1 h-2 w-2 shrink-0 rounded-full" style={{ background: color as string }} /><div><p className="text-sm font-medium text-slate-200">{title as string}</p><p className="mt-1 text-xs leading-5 text-slate-500">{note as string}</p></div></div>)}</div></Panel></div></>}

            {["Recommended Actions", "Route Planning", "Product Profiles", "Audit Reports"].includes(activeView) && <Panel className="mt-5 p-8"><p className="text-sm font-semibold text-white">{meta.title}</p><p className="mt-2 max-w-lg text-sm leading-6 text-slate-400">This workspace is available in the main operations view while the dedicated module is being prepared. Use the active shipment list to keep dispatch moving.</p><button type="button" onClick={() => changeView("Dashboard")} className="mt-5 min-h-10 rounded bg-[#238767] px-4 text-sm font-semibold text-white hover:bg-[#2c9c78]">Return to dashboard</button></Panel>}
          </div>
        </section>
      </div>
    </main>
